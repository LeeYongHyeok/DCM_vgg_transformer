...
def process_sample(aud_path, vid_path, lable, utt_id, sp, tgt_dict):
    input = {}
    output = {}
    si, ei = torchaudio.info(aud_path)
    input["length_ms"] = int(si.length / si.channels / si.rate / MILLISECONDS_TO_SECONDS)
    input["path_aud"] = aud_path
    input["path_vid"] = vid_path
...
def main():
    ...
    parser.add_argument("--video-dirs", nargs="+", default=['-'], required=True,
                        help="input directories with video files")
    ...
    labels = {}
    print("label making")
    for line in tqdm(args.labels):
        #(utt_id, label) = line.split(" ", 1)
        utt_id = line.split(" ")[0] 
        label = line.split(utt_id)[1].lstrip()
        labels[utt_id] = label
    if len(labels) == 0:
        raise Exception('No labels found in ', args.labels_path)

    Sample = namedtuple('Sample', 'aud_path vid_path utt_id')
    samples = []
    print("Sample start")
    for path, _, files in chain.from_iterable(os.walk(path) for path in args.audio_dirs):
        for f in files:
            if f.endswith(args.audio_format):
                ...
                paths = temp_path[-2]+'/'+temp_path[-1]
                utt_id = paths + '/' + filename
                v_path = path.replace('audio','video')
                if utt_id not in labels:
                    continue
                samples.append(Sample(os.path.join(path, f), os.path.join(v_path, filename+".npz") ,utt_id))

    utts = {}
    num_cpu = multiprocessing.cpu_count()
    print("json start")
    with concurrent.futures.ThreadPoolExecutor(max_workers=num_cpu) as executor:
        future_to_sample = {executor.submit(process_sample, s.aud_path, s.vid_path, labels[s.utt_id], s.utt_id, sp, tgt_dict): s for s in samples}
